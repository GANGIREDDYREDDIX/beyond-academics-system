...
import { Upload, Star, Trophy, Award, CheckCircle, FileText, Calendar, MapPin, Users, AlertCircle, Loader2 } from "lucide-react";
...
const achievementSchema = z.object({
  ...
  date: z.string()
    ...
    .refine((date) => {
      ...
      return achievementDate >= twoYearsAgo && achievementDate <= new Date();
    }, "Achievement must be within the last 2 years"),
  ...
  relatedLinks: z.string().url("Please enter a valid URL").optional().or(z.literal("")),
});
...
const BeyondAcademicsAddAchievement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      achievementSchema.parse(formData);
    } catch (error) {
    ...
    if (!selectedFile) {
      setErrors({ ...errors, file: "Please upload a certificate or proof document" });
      toast({
        title: "Certificate Required",
        description: "Please upload a certificate to verify your achievement.",
        variant: "destructive",
      });
      return;
    }
      ...
      toast({
        ...
        description: (
          <div className="flex items-center gap-2">
            ...
            <span>Your achievement has been sent for faculty verification. You can track the status in real-time.</span>
          </div>
        ),
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-subtle">
      ...
      <main id="main-content" className="container mx-auto px-4 py-8">
        ...
        {/* Progress Steps */}
        <nav aria-label="Form progress" className="mb-8 flex justify-center">
          <ol className="flex items-center space-x-4">
            ...
            <li className="w-8 h-px bg-muted" aria-hidden="true"></li>
            <li className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <CardHeader>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center">
                    {currentStep === 1 ? <FileText className="w-4 h-4 text-primary" aria-hidden="true" /> : 
                     currentStep === 2 ? <Upload className="w-4 h-4 text-primary" aria-hidden="true" /> : 
                  ...
                  {!isSubmitting && currentStep === 1 && (
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label htmlFor="school" className="text-sm font-medium">Your School *</Label>
                          <Select 
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="program" className="text-sm font-medium">Your Program *</Label>
                          <Select 
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            ...
                            <Input 
                              ...
                              className={`pl-10 h-10 ${errors.institution ? 'border-red-500' : ''}`}
                          ...
                          {errors.institution && <p id="institution-error" className="text-sm text-red-500" role="alert">{errors.institution}</p>}
                        </div>
                      </div>

                      <div className="space-y-4">
                        <Label className="text-sm font-medium" id="level-label">Competition/Achievement Level</Label>
                        <RadioGroup 
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <Trophy className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-muted-foreground" aria-hidden="true" />
                          <Input 
                            id="position" 
                            placeholder="e.g., 1st Place, Gold Medal, Winner"
                            className={`pl-10 h-10 ${errors.position ? 'border-red-500' : ''}`}
                        ...
                        {errors.position && <p id="position-error" className="text-sm text-red-500" role="alert">{errors.position}</p>}
                      </div>
                  ...
                  {!isSubmitting && currentStep === 2 && (
                    <>
                      <div className="space-y-2">
                        ...
                        <Textarea
                          id="description"
                          placeholder="Describe your achievement in detail. What did you do? What challenges did you overcome? What was your role? (Min 50 characters)"
                        ...
                        />
                        <div id="description-hint" className="flex justify-between text-xs text-muted-foreground" aria-live="polite">
                          <span>{formData.description.length}/1000 characters</span>
                          <span className={formData.description.length < 50 ? 'text-orange-500' : 'text-green-500'}>
                            {formData.description.length < 50 ? `${50 - formData.description.length} more needed` : '✓ Minimum met'}
                          </span>
                        </div>
                        {errors.description && <p id="description-error" className="text-sm text-red-500" role="alert">{errors.description}</p>}
                      </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-card/80 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="flex items-center text-lg">
                  <Trophy className="mr-2 text-primary w-5 h-5" aria-hidden="true" />
                  Recent Achievements
                </CardTitle>
